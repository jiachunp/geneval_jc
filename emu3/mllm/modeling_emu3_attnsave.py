...
def copy_and_mask_uncond_attn(attn_probs, flag_indexes, pad_value=-2):
    """
    attn_probs: (B, H, Q, K) after softmax
    flag_indexes: (B, K) or (B, 1, K), second sample (index 1) uses -2 to denote padding tokens.
    We assume B=2: [0]=cond, [1]=uncond.
    """
    B, K = flag_indexes.shape
    assert B == 2, "This helper assumes batch size 2 (cond + uncond)."
    ...
    if flag_indexes.dim() == 2:           # (B, K)
        ...
        uncond_flags = flag_indexes[1, 0] # (K,)
        pad_mask = ((uncond_flags == -2) | (uncond_flags == -1)) & (cond_flags == 0)  # (K,)
        pad_mask = pad_mask.view(1, K)  # (1,1,K)
    else:
        raise ValueError("flag_indexes must be (B,K) or (B,1,K)")
    
    # broadcast to (H, Q, K) when applied to new_attn[1]
    #  new_attn[1] has shape (H, Q, K)
    valid_mask = ~pad_mask  # True for non-pad positions

    # 3) Zero out padding positions in uncond attention
    new_attn = new_attn * valid_mask
    ...
    #import pdb; pdb.set_trace()
    index_mask = (flag_indexes[1] == -1) & (flag_indexes[0] == 0)
    idx = torch.nonzero(index_mask, as_tuple=False).squeeze(-1)
    
    k = idx.item()  # idx is a scalar tensor

    tmp = new_attn[:, :, k].clone()      # save column k
    new_attn[:, :, k] = new_attn[:, :, 0]   # put col 0 into k
    new_attn[:, :, 0] = tmp   
...
@dataclass
class CausalLMOutputWithPastAndFlags(CausalLMOutputWithPast):
    """
    Causal LM outputs with past key values and an extra flag tensor
    that you want to carry across generation steps.

    Inherits everything from `CausalLMOutputWithPast`:
    ...
    And adds:
      - flag_indexes: Optional[torch.Tensor]  # shape like (batch, seq_len)
    """
    flag_indexes: Optional[torch.Tensor] = None

@dataclass
class BaseModelOutputWithPastAndBefore(BaseModelOutputWithPast):
    """
    Causal LM outputs with past key values and an extra flag tensor
    that you want to carry across generation steps.
    ...
    And adds:
      - flag_indexes: Optional[torch.Tensor]  # shape like (batch, seq_len)
    """
    last_before_norm_hs: Optional[torch.FloatTensor] = None
...
class Emu3RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        after_hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon)
        return self.weight * after_hidden_states.to(input_dtype), hidden_states
...
class Emu3Attention(nn.Module):
    ...
    def forward(
        ...
        flag_indexes: Optional[torch.LongTensor] = None,
...
class Emu3Model(Emu3PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states, before_norm_hs = self.norm(hidden_states)
        ...
        return BaseModelOutputWithPastAndBefore(
            last_hidden_state=hidden_states, 
            past_key_values=next_cache,
            hidden_states=all_hidden_states,
            attentions=all_self_attns,
            last_before_norm_hs=before_norm_hs,
        )
...
class Emu3ForCausalLM(Emu3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.special_token_ids = torch.tensor([151849, 151850, 151851, 151852, 151853, 151846, 151847])
        self.pad_token_ids = torch.tensor([151643])
    ...
    @add_start_docstrings_to_model_forward(EMU3_INPUTS_DOCSTRING)
    # @replace_return_docstrings(output_type=CausalLMOutputWithPastAndFlags, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        flag_indexes:Optional[torch.FloatTensor] = None,
        ...
        return CausalLMOutputWithPastAndFlags(
            loss=loss,
            logits=logits,
    ...
    def prepare_inputs_for_generation(
        self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, flag_indexes=None, **kwargs
    ):
        ...
        next_flag = torch.ones_like(input_ids).to(input_ids.device)
        
        # condition 1: <= 151642 → 0
        next_flag[input_ids <= 151642] = 0

        # condition 2: special_token_ids → -1
        special_token_ids = self.special_token_ids.to(input_ids.device)
        pad_token_ids = self.pad_token_ids.to(input_ids.device)
        mask_special = torch.isin(input_ids, special_token_ids)
        next_flag[mask_special] = -1
        
        mask_pad = torch.isin(input_ids, pad_token_ids)
        next_flag[mask_pad] = -2
        
        # append to history (or init if first step)
        if flag_indexes is None:
            flag_indexes = next_flag
        else:
            flag_indexes = torch.cat([flag_indexes, next_flag], dim=1)

        # if we cropped attention_mask due to max_cache_length, crop flags too
        if attention_mask is not None and flag_indexes.shape[1] > attention_mask.shape[1]:
            flag_indexes = flag_indexes[:, -attention_mask.shape[1]:]
    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs, **kwargs):
        # let the base class do its normal updates (attention_mask, past_key_values, etc.)
        model_kwargs = super()._update_model_kwargs_for_generation(outputs, model_kwargs, **kwargs)

        # now add our custom tensor
        if getattr(outputs, "flag_indexes", None) is not None:
            model_kwargs["flag_indexes"] = outputs.flag_indexes
