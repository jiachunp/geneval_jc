...
# Efficient implementation equivalent to the following:
def scaled_dot_product_reattention(query, key, value, attn_mask=None, dropout_p=0.0,
        is_causal=False, scale=None, enable_gqa=False) -> torch.Tensor:
    L, S = query.size(-2), key.size(-2)
    scale_factor = 1 / math.sqrt(query.size(-1)) if scale is None else scale
    attn_bias = torch.zeros(L, S, dtype=query.dtype, device=query.device)
    ...
    attn_weight = query_s @ key_s.transpose(-2, -1) * scale_factor
    #attn_weight = query @ key.transpose(-2, -1) * scale_factor

    attn_weight_uncond = torch.full_like(attn_weight, float("-inf"))
    attn_weight_uncond[:,:4096,:4096] = attn_weight[:,:4096,:4096]
    attn_weight_uncond[:,4096:,4096:] = attn_weight[:,4096:, 4096:]
    attn_weight_full = torch.stack([attn_weight_uncond, attn_weight], dim=0)

    attn_weight_full += attn_bias
    attn_weight_full = torch.softmax(attn_weight_full, dim=-1)

    attn_weight_full = torch.dropout(attn_weight_full, dropout_p, train=True)
    return attn_weight_full @ value
